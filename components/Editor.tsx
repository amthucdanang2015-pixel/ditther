...
export default function Editor() {
  ...
  const filterGroups = [
    ...
    {
      title: "Vignette",
      filters: [{ name: "Vignette", value: 50, max: 100, suffix: "" }],
    },
            ...
            <div className="grid grid-cols-5 gap-[6px]">
              {backgrounds.map((src) => (
                <button
                  ...
                  className={`relative aspect-square overflow-hidden rounded-xl transition-all duration-200 hover:scale-[0.96] ${selectedBg === src ? "ring-2 ring-white" : ""}`}
                >
                  <Image
                    src={src}
                    alt=""
                    ...
                    {group.filters.map((filter) => (
                      <div key={filter.name}>
                        <div className="flex justify-between mb-2">
                          <span className="text-[15px]">{filter.name}</span>
